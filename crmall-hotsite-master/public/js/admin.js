...
function onInputChangeEvent() {
    $(".winnerInputColumn").keyup(function () {
        ...
        var $allInputs = $(".winnerInputColumn");
...
function registerExpandCampaignEvent() {
    $(".lotteryWinnersContainer").on('show.bs.collapse', function () {
        var $btnCircle = $(this).prev().find('.btn-circle');
...
function confirmRemoveWinnerEvent() {
    $("#formConfirmRemove").click(function () {
        ...
        $.post('/adm/remover-ganhador', window.formParams).done(function (response) {
            var urlRedirect = '/adm/ganhadores';
            var campaignId = response.campaignId;
...
function registerSaveWinnerEvent() {
    $(".btnSaveWinner").click(function () {
        var $parentLine = $(this).parent().parent().parent();
        var $tbody = $parentLine.parent();
        var $lineForm = $tbody.find("#lineForm");

        var formParams = {};

        var $inputs = $parentLine.find('input');

        for (var i = 0; i < $inputs.length; ++i) {
            var inputName = $($inputs[i])[0].dataset.name;
...
function confirmSaveWinnerEvent() {
    $("#formConfirmSave").click(function () {
        ...
        $.post('/adm/adicionar-ganhador', window.formParams).done(function (response) {
            var urlRedirect = '/adm/ganhadores';
