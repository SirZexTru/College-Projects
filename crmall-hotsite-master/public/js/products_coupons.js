...
function initProductsList() {
    if (hasProductsList) {
        ...
        $("#btnAddProduct").click(function () {
            var quantity = parseInt($("#quantityNumber").val());
            var value = parseFloat($("#productValue").val().replace(/\./gi, '').replace(/,/gi, '.')) * quantity;
            var product = $('#productSearch').val().split('_');
            var totalProducts = $('.productSelectionContainer.result #totalProducts');
            var totalValue = $('.productSelectionContainer.result #totalValue');
            var valueMaskMoney = 0;

            if (typeof product[0] === 'string' || product[0] instanceof String) {
                var numbers = product[0].match(/\d/g);
            ...
            if (quantity > 0) {
                ...
                if ($('#product-' + product[0]).length) {
                    var actualQtd = $('#product-' + product[0] + ' .qtd-value');
                    var actualValue = $('#product-' + product[0] + ' .value');

                    var qtdCalculated = parseInt(actualQtd.text()) + quantity;
                    var qtdValue = parseFloat(products[product[0]].productValue) + value;
        ...
        $(".plus").click(function () {
            if (!$(this).hasClass('disabled')) {
                var $quantityElem = $(this).prev().find('input');
        ...
        $(".minus").click(function () {
            if (!$(this).hasClass('disabled')) {
                var $quantityElem = $(this).next().find('input');
                var quantity = parseInt($quantityElem.val());
...
$('input[name=question]').change(function () {
    var question = $('input[name=question]:checked').val() === 'riogaleao';
...
function submitForm(form, event) {
    ...
    if (!allowSubmitForm || !inputImage.val() || !$(form).valid() || !question || $('#boughtDate').hasClass('error-custom') || $('#receiptValue').hasClass('error-custom')) {
    ...
    var url = '/meus-cupons/send-invoice';
...
$("#image").change(function () {
    ...
    var fileInput = $('#image');
    var messageFileMax = $('#fileSizeMax');
    var messageImageRequired = $('#imageRequired');
    var messageExtensionInvalid = $('#extensionInvalid');
    var fileObj = fileInput.get(0).files[0];

    if (fileObj) {
        var fileSize = fileObj.size; // in bytes
